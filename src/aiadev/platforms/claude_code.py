...
* ``AGENTS.md`` at the project root — the canonical agent file (spec
  0016 Story 3, ADR-5). This is where the engine writes the full
  preset content, including the ``<!-- aiadev:auto-stack -->`` block
  regenerated by ``aiadev sync``. Claude Code itself reads ``AGENTS.md``
  natively; the framework also drops a thin ``CLAUDE.md`` wrapper (see
  :func:`wrapper_target` / :func:`render_wrapper`) for tooling that
  still looks for that filename specifically.
