...
Writes ``AGENTS.md`` at the project root — the canonical agent file
(spec 0016 Story 3, ADR-5): the engine writes the full preset content
there, including the ``<!-- aiadev:auto-stack -->`` block regenerated by
``aiadev sync``. The handler also drops a thin ``GEMINI.md`` wrapper
(see :func:`wrapper_target` / :func:`render_wrapper`) since Gemini CLI's
own convention looks for that filename; the wrapper just points at
``AGENTS.md``. Skills live under ``.gemini/skills/<skill-name>/SKILL.md``.
...
def wrapper_target(
    ...
    *,
    ...
    """Return the thin-wrapper path for ``role``, or ``None`` if it has none.

    Spec 0016 Story 3 / ADR-5: the engine writes the full generated
    content to :func:`resolve_target`'s ``AGENTS.md`` and, when this hook
    returns a path, additionally writes a short pointer file there. Only
    ``agent_file`` has a wrapper; project scope only (agent_file is not
    installable at user scope in the first place).
    """
    if role != "agent_file" or scope != "project":
        return None
    return install_root / "GEMINI.md"


def render_wrapper(role: ArtifactRole, canonical_name: str) -> str | None:
    """Return the wrapper file's content for ``role``, or ``None``.

    Deterministic and byte-stable — the wrapper never carries generated
    content (no stack block, no timestamps) so re-installing/re-syncing
    never touches it once it matches.
    """
    if role != "agent_file":
        return None
    return (
        f"# {canonical_name}\n\n"
        f"Project guidance for agents and humans lives in "
        f"[`{canonical_name}`](./{canonical_name}). Read that file first.\n"
    )
