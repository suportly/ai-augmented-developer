"""Tests for :mod:`aiadev.platforms.codex`."""
...
from aiadev.platforms import codex
...
class TestResolveTarget:
    def test_agent_file(self, tmp_path: pathlib.Path) -> None:
        assert codex.resolve_target("agent_file", "", tmp_path) == tmp_path / "AGENTS.md"

    def test_constitution(self, tmp_path: pathlib.Path) -> None:
        assert codex.resolve_target("constitution", "", tmp_path) == tmp_path / "constitution.md"

    def test_skill(self, tmp_path: pathlib.Path) -> None:
        assert codex.resolve_target("skill", "hello", tmp_path) == (
            tmp_path / ".codex" / "skills" / "hello" / "SKILL.md"
        )

    def test_skill_without_name_rejected(self, tmp_path: pathlib.Path) -> None:
        with pytest.raises(ValueError, match="non-empty name"):
            codex.resolve_target("skill", "", tmp_path)
...
class TestIterPresetArtifacts:
    ...
    def test_empty(self, tmp_path: pathlib.Path) -> None:
        assert list(codex.iter_preset_artifacts(tmp_path)) == []
    ...
    def test_constitution_included(self, tmp_path: pathlib.Path) -> None:
        ...
        roles = [r for (r, _n, _p) in codex.iter_preset_artifacts(tmp_path)]
    ...
    def test_skills_sorted(self, tmp_path: pathlib.Path) -> None:
        ...
        names = [n for (_r, n, _p) in codex.iter_preset_artifacts(tmp_path)]
